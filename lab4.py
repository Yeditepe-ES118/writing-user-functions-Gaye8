...
def throw_rock (m, v0, theta):
    ...
    tf = 2* v0 * np.sin (theta)
    R = v0** 2 * np.sin(2 * theta) / g # in m
    hm = v0 **2 * np.sin(theta)**2 /(2 * g) # in m 
    vh = v0 * np.cos(theta) # in m/s
    Kh = 0.5 * m * vh * hm**2 # in J
    print ("For a rock with %5.3f kg mass thrown with %5.3f m/s is at an angle of %6.2f degrees:\n"
           ...
           "Kinetic energy at the maximum height is %8.2 J" % (m, v0, theta*180, tf, R, hm, vh, Kh ))
